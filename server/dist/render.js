...
export async function renderProject(project) {
    ...
    if (project.audioBitrate)
        videoOptions.audioBitrate = project.audioBitrate;
    if (project.pixelFormat)
        videoOptions.pixelFormat = project.pixelFormat;
    if (project.volume !== 100)
        videoOptions.volume = project.volume;
    if (project.coverCapture) {
        videoOptions.coverCapture = true;
        if (project.coverCaptureTime > 0)
            videoOptions.coverCaptureTime = project.coverCaptureTime;
        if (project.coverCaptureFormat)
            videoOptions.coverCaptureFormat = project.coverCaptureFormat;
    }
    ...
    const video = wvc.createMultiVideo(videoOptions);
    ...
    const resolvedAudios = await Promise.all(validAudios.map(async (a) => {
